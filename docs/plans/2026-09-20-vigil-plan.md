...
false. Automatic peer enforcement covers numeric Tailscale IPv4/IPv6 target
hosts only. Schema validation does not resolve DNS; owners must mark remote
...
Command arguments are literal argv entries passed to `execFile` without a shell.
Metacharacters and spaces are data; no variable or command expansion occurs.
The executable must exactly match the absolute-path allowlist. Rejecting shell
punctuation in data arguments is not part of the schema.

All network and subprocess checks have a 10-second hard deadline (tcp 5 seconds),
bounded output (64 KiB), and no inherited interactive stdin. Status-only HTTP
checks resolve from the response headers and close the response without buffering
the body; body/JSON assertions retain the 64 KiB cap. Check at most four
...
- [ ] CLAUDE.md: extend the documented direct-bind exception paragraph to name `vigil-tick` alongside `sancta-gallery`, same reasoning (an absent socket listener refuses connections; a failed socket-activated handler may reset or time out, and bounded HTTP/freshness checks expose either failure instead of Serve's unrelated 200).
...
- [ ] `hosts/rpi5-full/configuration.nix`: `services.vigil = { enable = true; contractsDirs = [ ./vigil-contracts ]; expectedContracts = 7; telegramEnvFile = secret "backup-telegram-env"; hassTokenFile = secret "ha-vigil-token"; hassUrl = "http://127.0.0.1:8123"; listenAddress = "100.106.93.87"; cmdAllow = []; }`; `age.secrets.ha-vigil-token = { …; owner = "vigil"; }`; keep `backup-telegram-env` root-owned with mode `0400` for systemd EnvironmentFile loading and its existing root consumers (`backup-pull`, `tailscale-dns-watchdog`). The status responder masks agenix storage, configured credential paths and private runtime contract directories.
