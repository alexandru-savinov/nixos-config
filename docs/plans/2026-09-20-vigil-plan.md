...
v2 of this plan (2026-09-20): v1 went through one revmux round (three Opus
lenses + Codex `gpt-5.6-sol` adversarial): 31 findings, all folded in below; the
disposition table is at the end. Design: private, on the soul volume
...
- A **contract** is one TOML file, one target. Schema (closed; unknown keys are
  ...
  "nota"` (`nota` = status row only, never Telegram). Check types are a closed
...
- **Transitions → say.** `vigil-check` is the only producer: on `open`,
  `escalation`, `close` (and the NECITIT `nota`) it invokes `vigil say` with one
  JSON event. If the send fails, the event is spooled to
...
- **explain** (choir only) is a SEPARATE unit `vigil-explain` running as
  ...
  writes `{nume, concluzie}` to `/var/lib/vigil/conclusions/<nume>.json`; the
  next `vigil check` tick picks the conclusion up and sends it. No credential
...
- **say** = Telegram (`EnvironmentFile` = the existing `backup-telegram-env`
  ...
  `getMe` and touches `/var/lib/vigil/last-channel-ok` on success; the
  `channel` contract is an `age` on that absolute path (`prag = "2d"`).
- **Tick.** `vigil-check` writes `/var/lib/vigil/tick` via `ExecStopPost=` (so it
  is refreshed on every run regardless of verdicts): a JSON body
  `{"la":"<ISO>","verde":n,"picat":n,"necitit":n}` — counts only, no names. A
...
- [ ] Transitions: for each `open` / `escalation` / `close` / NECITIT-nota, build one JSON event `{gazda, nume, verdict, tranzitie, nivel, concluzie?}` (never `tinta`, never journal text) and spawn `vigil say` with it on stdin; on non-zero exit, write the event to `/var/lib/vigil/outbox/<ts>-<nume>.json`; at the start of every tick, retry outbox events younger than 24 h and delete those that succeed or age out. Also: if `/var/lib/vigil/conclusions/<nume>.json` exists for an open incident, attach `concluzie` to the next event for that contract and delete the file. Once per 24 h (marker `/var/lib/vigil/last-getme`), call Telegram `getMe` through `vigil say --getme`; success touches `/var/lib/vigil/last-channel-ok`.
...
- [ ] `pkgs/vigil/vigil-say.mjs`: reads one JSON event on stdin; **refuses** (exit 2, no send) if the event has a key `jurnal` or `tinta`, or any string field longer than 500 chars. `nivel = "nota"` → status row only, no Telegram. Otherwise `sendMessage` to `$TELEGRAM_CHAT_ID` with `$TELEGRAM_BOT_TOKEN` (from the unit's `EnvironmentFile`; never logged): `<emoji by nivel/tranzitie> [<gazda>] <nume>: <verdict> — <tranzitie>` + optional one-line `concluzie`. Assert `"ok":true`; success → touch `/var/lib/vigil/last-say-ok` and `last-channel-ok`; failure → exit 1 (the caller spools). `--getme` mode: `getMe`, assert `"ok":true`, touch `last-channel-ok`, exit 0/1.
...
- [ ] `pkgs/vigil/vigil-tick.mjs`: `write` mode — write `/var/lib/vigil/tick` as the JSON body from Context (counts only; no contract names); `serve` mode — stdin/stdout only (systemd socket activation, `Accept=yes`): reply `HTTP/1.0 200` + body if the file exists, `404` if absent, close. No listening code in the program.
- [ ] `--autoproba` for say: against a local `http.createServer` playing Telegram (`ok:true` / `ok:false` / 429 / refused); assert `last-say-ok` is touched only on ok, `jurnal`/`tinta` keys refused, a 501-char field refused, `nota` never hits the fake server, `--getme` touches `last-channel-ok`. For tick: pipe a request through `serve` and assert body equals file; absent file → 404.
...
- [ ] **PR 5b** (only after the owner confirms the re-key): set `telegramEnvFile = secret "backup-telegram-env"` (owner/group as decided in Task 4), declare `age.secrets.backup-telegram-env` on choir, add `channel.toml`, `expectedContracts = 11`. Closing check (his hand): 11 contracts; `last-channel-ok` exists within a day; a deliberate probe (a temporary public contract pointing at a closed port, removed in the same PR series) produces exactly one Telegram message on open and one on close.
...
- [ ] The proof of right #4 runs **inside `vigil.service`, as `vigil`, in `--autoproba`** — no hand-run as `vigil` (a system user cannot be impersonated without root). The module declares two probe units: `vigil-probe.service` (`Type=oneshot`, `RemainAfterExit=true`, `ExecStart = "${pkgs.coreutils}/bin/true"`; **in** the derived set via a probe contract) and `vigil-probe-denied.service` (same, **not** in any contract). Positive arm: restart of `vigil-probe` succeeds; negative arm: restart of `vigil-probe-denied` is refused by polkit. If both are refused → the autoproba reports `polkit` as the cause (rule not installed) and exits 2 — the owner's-hand alternative (`sudo systemctl restart vigil-probe.service` as a sanity check of the unit itself) is written in the PR body.
- [ ] `vigil-check` gains the journal step (it is the journal-group account): at incident **open** for a contract with `jurnal`, run `journalctl -u <unit> --since -15min -o cat` per unit (absolute path; **check the exit status; non-zero → the slice is `NECITIT: journal` and says so**), pass through `lib/redact.mjs` (deny-list from Context + IP-literal stripping + the contract's `jurnal_filtru` if set), write `/var/lib/vigil/journal/<nume>-<ts>.txt` (mode 0644).
...
- [ ] `--autoproba`: repair — positive and negative polkit arms above, plus budget exhaustion → escalation; explain — a fixture slice containing every deny-listed class and an IP → none survives; a 501-char model output → truncated; an already-explained incident → the command is **not** invoked. Mutants: repair ignores per-incident `max` (→ red); one deny-list entry disabled (→ red); explain runs on a missing slice (→ red).
- [ ] **Wire it:** add `[recuperare]` (`act = "/run/current-system/sw/bin/systemctl restart sancta-gallery.service"`, `max = 1`) to `galeria.toml` and the same for `membrana.toml`; add the probe contract `vigil-probe.toml` with `jurnal = [ "vigil-probe.service" ]` and a `[recuperare]` on `vigil-probe.service`; make the probe unit, when started with `VIGIL_PROBE_FAIL=1`, log one line of each deny-listed class plus an IP literal and then exit 1. Enable `explain` on choir with the chosen `explain.command`. `absenta` clean; module-eval green; PR. **Closing check (his hand):** trigger the probe failure; within two ticks: one Telegram message with a conclusion; `/var/lib/vigil/journal/vigil-probe-*.txt` is **non-empty, contains the probe's benign line, and contains none of the planted ones**; `decisions.jsonl` shows the probe restart; `vigil-probe-denied` was refused in the autoproba; an ack drop-file clears an escalated probe incident on the next tick.

### Task 7: the private contracts on rpi5, and the first real incident
- [ ] Once the owner has created the three `.age` files: declare `age.secrets.vigil-rpi5-contract-{1,2,3} = { file = …; owner = "vigil"; path = "/run/vigil-contracts/contract-N.toml"; }`, add `"/run/vigil-contracts"` to `contractsDirs`, `expectedContracts = 10`. PR, CI, his switch. Closing check: `journalctl -u vigil` shows **10 contracts, zero NECITIT** (the three private ones with real verdicts); the count assertion is what proves the files were parsed, not merely present.
