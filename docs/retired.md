...
| `checks.x86_64-linux.openclaw-zdr-proxy` + `tests/openclaw-zdr-proxy.nix` (CI check) | 2026-08-20 | Not itself host-coupled (self-contained VM+stub), but its only *production* consumer — `hosts/sancta-claw/openclaw-service.nix` — is gone, so it was testing a module nothing deploys anymore. At the time, `modules/services/openclaw-zdr-proxy.nix` and `modules/services/openclaw.nix` were kept, still covered by `tests/module-eval.nix` in isolation — see the row below for the owner call that later removed them too. | `13e6f1086b418f6d869902d23de61d12044c756c` | `git show 13e6f1086b418f6d869902d23de61d12044c756c:tests/openclaw-zdr-proxy.nix` + restore the `checks.x86_64-linux.openclaw-zdr-proxy` block in `flake.nix`. |
| `modules/services/{openclaw,openclaw-zdr-proxy,nullclaw,claude}.nix` (modules) + their `tests/module-eval.nix` cases + `tests/test_openclaw_zdr_proxy.py` | 2026-08-22 | Faza 2 repo census flagged all four as unreferenced by any `hosts/*/configuration.nix` (their only production consumers were the already-destroyed `sancta-claw`/`zero-kuzea` hosts, see rows above). Re-verification before deletion found each still had a live import from `tests/module-eval.nix` — the isolated module-eval tests that the row above says "still covered" them — plus `openclaw-zdr-proxy.nix` had a live Python test (`tests/test_openclaw_zdr_proxy.py`) importing its submodule. This is the owner call the row above deferred: the modules, their `tests/module-eval.nix` cases (11 total), and the Python test were all removed together, since none of that coverage exercises capability any host in the fleet still runs. | `64deab1e1d313d2e8ddd1de5c2ad0f4e62f7824b` (tip of `origin/main` immediately before PR #571) | `git show 64deab1e1d313d2e8ddd1de5c2ad0f4e62f7824b:modules/services/openclaw.nix` (repeat per file/path above) + restore the matching test blocks from the same commit in `tests/module-eval.nix` and restore `tests/test_openclaw_zdr_proxy.py` from the same commit. |
