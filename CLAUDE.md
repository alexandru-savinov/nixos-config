...
Vigil's network design uses the same direct-bind exception for `vigil-tick`: its socket binds the configured Tailscale address on port 8747, with the Tailscale-only IP address filter on the socket unit. An absent socket listener fails as `ECONNREFUSED`; a failed socket-activated handler can instead reset the connection or time out. Bounded HTTP and freshness checks expose these failures rather than accepting an unrelated 200 from Serve's catch-all. The runtime/rpi5 wiring is reviewed in PR #595; choir row-only wiring is in dependent PR #596. Merge #595 first, then rebase #596 onto main before its merge. The dependent PR targets main for CI, so its pre-rebase diff also includes the parent commits. Deployment and live socket acceptance on both hosts remain owner steps; verify their status using `docs/plans/2026-09-20-vigil-handoff.md` before relying on either endpoint.
